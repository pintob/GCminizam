...
mlvalue* free_list_new(size_t size, GC_global_data* data){
    ...
    ajust_cell(&(data->free_list), &ptr, size/8);
...
void new_page(GC_global_data* data){
    Page elem = calloc(1, sizeof(Elem_list) + sizeof(mlvalue) +sizeof(Page) + 64 * KB);
    set_size(&elem->cell, 64 * KB / 8);
...
void display_free_list(GC_global_data* data){
    for(Elem_list e = data->free_list; e != NULL; e = e->next){
        printf("%p: %ld %ld\n", e, Color_hd(e->header), Size_hd(e->header));
    }
}
...
void ajust_cell(Elem_list* free_list, Elem_list* elem_to_ajust, int size){
    ...
    Elem_list elem = (Elem_list )(((uint64_t) ((*elem_to_ajust)->body)) + size * 8);
    set_size(elem, Size_hd((*elem_to_ajust)->header) - size - 3);
...
void remove_page_elem(Page list, Elem_list* free_list){
    ...
    *free_list = NULL;
    ...
    for(Page p = list; p != NULL; p = p->next){
        e = &(p->cell); i = 0;
        while(i < 64 * KB / 8) {
            Elem_list temp = (Elem_list) ((uint64_t) e + 24 + Size_hd(e->header)*8);

            if((uint64_t) temp == ((uint64_t)p) + 64 * KB)
                break;

            if(i + Size_hd(e->header) < (64 * KB / 8) - 3 &&
                (Color_hd(temp->header) == WHITE && Color_hd(e->header) == WHITE)){
                set_size(e, 3 + Size_hd(e->header) + Size_hd(temp->header));
            }
            else {

                if(Color_hd(e->header) == WHITE){
                    e->next = *free_list; // insertion in head
                    *free_list = e;
                }
                (e->header) &= ~(3 << 7);
                (e->header) |= (WHITE << 8);
                i += Size_hd(e->header) + 3;
                e = temp;
            }
        }
